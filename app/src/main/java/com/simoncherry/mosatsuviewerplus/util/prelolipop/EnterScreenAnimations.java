...
public class EnterScreenAnimations extends ScreenAnimation {
    ...
    private float[] mTargetImageMatrixValues = new float[9];
    ...
    public void setEnterAnimationListener(EnterAnimationListener listener) {
        enterAnimationListener = listener;
    }
            ...
            public void onAnimationEnd(Animator animation) {
                ...
                if (mEnteringAnimation != null) {
                    ...
                    if (enterAnimationListener != null) {
                        enterAnimationListener.onAnimationEnd();
                    }
                } else {
    ...
    private ObjectAnimator createEnteringImageMatrixAnimator() {
        ...
        endMatrix.getValues(mTargetImageMatrixValues);
        //Logger.t(TAG).e("createEnteringImageMatrixAnimator, mInitThumbnailMatrixValues " + Arrays.toString(mInitThumbnailMatrixValues));

        Logger.t(TAG).e("createEnteringImageMatrixAnimator, initMatrix " + initMatrix);
        Logger.t(TAG).e("createEnteringImageMatrixAnimator,  endMatrix " + endMatrix);
    ...
    public float[] getTargetImageMatrixValues() {
        return mTargetImageMatrixValues;
    }

    public void setInitThumbnailMatrixValues(ImageView imageView) {
        Matrix initMatrix = MatrixUtils.getImageMatrix(imageView);
        initMatrix.getValues(mInitThumbnailMatrixValues);
    }

    public void setTargetImageMatrixValues(ImageView imageView) {
        Matrix initMatrix = MatrixUtils.getImageMatrix(imageView);
        initMatrix.getValues(mTargetImageMatrixValues);
    }
